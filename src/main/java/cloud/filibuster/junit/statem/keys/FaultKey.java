...
public class FaultKey<ReqT, ResT> {
    ...
    public FaultKey(Builder builder) {
    ...
    public FaultKey(MethodDescriptor<ReqT, ResT> methodDescriptor) {
        this.method = methodDescriptor.getFullMethodName();
    }

    public FaultKey(MethodDescriptor<ReqT, ResT> methodDescriptor, Status.Code code) {
        this.faultKeyType = METHOD_AND_CODE;
        this.method = methodDescriptor.getFullMethodName();
        this.code = code.toString();
    }

    public FaultKey(MethodDescriptor<ReqT, ResT> methodDescriptor, ReqT request) {
        this.faultKeyType = METHOD_AND_REQUEST;
        this.method = methodDescriptor.getFullMethodName();
        this.request = request.toString();
    }

    public FaultKey(MethodDescriptor<ReqT, ResT> methodDescriptor, Status.Code code, ReqT request) {
    ...
    public static List<FaultKey> generateFaultKeysInDecreasingGranularity(JSONObject rpcWhereFaultsInjected) {
        List<FaultKey> listOfFaultKeys = new ArrayList<>();
        ...
        listOfFaultKeys.add(
                new FaultKey.Builder().setType(METHOD_REQUEST_AND_CODE).setMethod(method).setRequest(request).setCode(code).build()
        );
        listOfFaultKeys.add(
                new FaultKey.Builder().setType(METHOD_AND_REQUEST).setMethod(method).setRequest(request).build()
        );
        listOfFaultKeys.add(
                new FaultKey.Builder().setType(METHOD_AND_CODE).setMethod(method).setCode(code).build()
        );
        listOfFaultKeys.add(
                new FaultKey.Builder().setType(METHOD).setMethod(method).build()
        );
